...
SOURCES = [
    ...
    {"type": "channel", "id": "UC6ZFN9Tx6xh-skXCuRHCDpQ", "name": "PBSNewsHour", "cat": "news"},
...
TO = datetime(2018, 5, 1) + timedelta(seconds=-1)
...
CSV_PATH = "/Users/francois/Google Drive/these_pauline/csv2/"
...
def get_csv_comment(comment_file):
    ...
    comment_writer.writerow(["videoId", "commentId", "author", "text", "replies", "likes", "publishedAt"])
...
def get_last_situation():
    last_situation_file = Path("situation.txt")
    last = None
    situations = []
    if last_situation_file.is_file():
        with open('situation.txt', "r") as last_situation_file:
...
def write_situation(last_situation, period, source):
    with open('situation.txt', "a") as situation_file:
